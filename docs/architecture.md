...
- Optional local inference can affect final quality even when its tokens are unscored, so it must remain disabled by default until routing risk, fallback behavior, Docker impact, dependency impact, and runtime reliability are measured.
